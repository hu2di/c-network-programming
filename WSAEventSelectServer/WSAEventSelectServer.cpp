...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while(1)
	{
		ret = WSAWaitForMultipleEvents(nClient,
			events,
		...
		{
			ret = WSAWaitForMultipleEvents(1,&events[i],
				TRUE,1,FALSE);
			if (ret==WSA_WAIT_FAILED || 
				ret == WSA_WAIT_TIMEOUT) continue;
			WSAEnumNetworkEvents(clients[i],
				events[i],&networkEvents);
			...
			if (networkEvents.lNetworkEvents & FD_ACCEPT)
			{
				...
				else
				{
					...
					clients[nClient] = accept(server,
						(sockaddr*)&clientAddr,&clientAddrLen);
					events[nClient] = WSACreateEvent();
					WSAEventSelect(clients[nClient],events[nClient],
						FD_READ|FD_CLOSE);
